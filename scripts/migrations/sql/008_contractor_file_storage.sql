...
-- Step 1: Create dedicated file storage table (without foreign key initially)
CREATE TABLE IF NOT EXISTS contractor_file_storage (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    document_id INTEGER, -- Will add foreign key after contractor_documents table is updated
...
-- Add storage_id column without foreign key constraint first
ALTER TABLE contractor_documents
ADD COLUMN IF NOT EXISTS storage_id UUID;

-- Now add the foreign key constraint to contractor_file_storage
...
-- Add foreign key constraint to contractor_documents
ALTER TABLE contractor_documents
