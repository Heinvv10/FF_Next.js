 ...
 * Chart showing most used templates with stats
 */

import React from 'react';
import type { WorkflowAnalytics } from '../../../types/workflow.types';
import { CHART_COLORS } from '../data/colorSchemes';
...
export function TemplateUsageChart({ templateUsage }: TemplateUsageChartProps) {
  const maxProjectCount = templateUsage[0]?.projectCount || 1;
        ...
        {templateUsage.slice(0, 5).map((template, index) => (
          <div key={template.templateId} className="flex items-center space-x-4">
            <div className="flex items-center space-x-2 flex-1">
              <div className={`w-3 h-3 rounded-full ${CHART_COLORS[index] || CHART_COLORS[4]}`} />
            ...
            <div className="w-24 bg-gray-200 dark:bg-gray-700 rounded-full h-2">
              <div
                className={`h-2 rounded-full ${CHART_COLORS[index] || CHART_COLORS[4]}`}
                style={{
                  width: `${Math.min(100, (template.projectCount / maxProjectCount) * 100)}%`
                }}
              />
            </div>
          </div>
