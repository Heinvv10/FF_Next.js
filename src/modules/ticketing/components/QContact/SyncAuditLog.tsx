...
function getStatusIcon(status: SyncStatus): { Icon: React.ElementType; color: string } {
  switch (status) {
    ...
    default:
      return { Icon: Clock, color: 'text-white/60' };
  }
}
...
function getDirectionIcon(direction: SyncDirection): { Icon: React.ElementType; color: string } {
  switch (direction) {
    ...
    default:
      return { Icon: ArrowLeftRight, color: 'text-white/60' };
  }
}
...
function LogEntry({ log }: { log: QContactSyncLog }) {
  ...
  return (
    <div className="border border-white/10 rounded-lg bg-white/5 hover:bg-white/10 transition-all">
        ...
        <div className="flex-1 grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xs text-white/60">{formatSyncType(log.sync_type as SyncType)}</p>
            </div>
          </div>
          ...
          <div className="col-span-2">
            <div className="space-y-1 text-xs">
              {log.ticket_id && (
                <div className="flex items-center">
                  <span className="text-white/60 mr-2">FF Ticket:</span>
                  <span className="text-white font-mono">{log.ticket_id.slice(0, 8)}...</span>
                </div>
              )}
              {log.qcontact_ticket_id && (
                <div className="flex items-center">
                  <span className="text-white/60 mr-2">QContact ID:</span>
                  <span className="text-white font-mono">{log.qcontact_ticket_id}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="text-right">
            <Clock className="w-4 h-4 inline mr-1 text-white/60" />
            <span className="text-xs text-white/60">{formatTimestamp(log.synced_at)}</span>
          </div>
        </div>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-white/10">
          ...
          {log.request_payload && (
            <div className="mt-4">
              <p className="text-sm font-semibold text-white mb-2">Request Payload</p>
              <pre className="p-3 bg-white/5 border border-white/10 rounded-lg text-xs text-white/80 overflow-x-auto">
          ...
          {log.response_payload && (
            <div className="mt-4">
              <p className="text-sm font-semibold text-white mb-2">Response Payload</p>
              <pre className="p-3 bg-white/5 border border-white/10 rounded-lg text-xs text-white/80 overflow-x-auto">
  ...
  return (
    <div className="space-y-4">
      <button
        ...
        className="flex items-center text-white hover:text-white/80 transition-all"
      >
      ...
      {showFilters && (
        <div className="p-4 bg-white/5 border border-white/10 rounded-lg space-y-4">
          ...
          {/* Sync Type Filter */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">Sync Type</label>
            <select
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-white/10 border border-white/10 rounded-lg text-white text-sm placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-blue-500"
            />
          </div>

          {/* QContact ID Filter */}
          <div>
            <label className="block text-sm font-medium text-white mb-2">QContact Ticket ID</label>
            <input
              type="text"
    ...
    return (
      <div className="flex items-center justify-center p-12">
        <div className="text-center">
          <FileText className="w-8 h-8 text-white/60 animate-pulse mx-auto mb-4" />
          <p className="text-white/60">Loading sync logs...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <div className="p-12 bg-white/5 border border-white/10 rounded-lg text-center">
          ...
          <p className="text-white/60">No sync logs found</p>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-bold text-white mb-1">Sync Audit Log</h3>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-3 bg-blue-500/10 border border-blue-500/20 rounded-lg">
          <div className="flex items-center mb-1">
            ...
            <span className="text-xs text-white/60">Inbound</span>
          </div>
        ...
        <div className="p-3 bg-purple-500/10 border border-purple-500/20 rounded-lg">
          <div className="flex items-center mb-1">
            ...
            <span className="text-xs text-white/60">Outbound</span>
          </div>
