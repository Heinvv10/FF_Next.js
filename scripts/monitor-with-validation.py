...
class DropMonitor:
    ...
    def get_new_messages(self, group_jid: str) -> List[Dict]:
        ...
        try:
            ...
            # No sender filtering - we filter bot messages by content instead
            query = """
    ...
    def process_message(self, message: Dict, project_name: str, group_jid: str) -> bool:
        ...
        if project_name in ['Mohadin', 'Lawley', 'Velo Test', 'Mamelodi']:
            ...
            logger.debug(f"⏭️  SKIPPED VALIDATION: {drop_number} (no validation configured for {project_name})")
    ...
    def scan_project(self, project: Dict) -> int:
        ...
        for message in messages:
            # Skip bot's own rejection messages (content-based filter to prevent spam loops)
            content = message.get('content', '')
            if content and ('❌ Invalid Drop Number' in content or 'not in the valid list for' in content):
                logger.debug(f"⏭️  Skipping bot rejection message: {content[:50]}...")
                # Still update timestamp so we don't reprocess
