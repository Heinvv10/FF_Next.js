/**
 * PhasePerformanceCard Component
 * Displays phase-level performance metrics
 */
...
export function PhasePerformanceCard({ phaseMetrics }: PhasePerformanceCardProps) {
  const highRiskPhases = phaseMetrics.filter(p => (p.bottleneckRisk as string) === 'high').length;
    ...
    <div className="bg-white dark:bg-gray-900 rounded-lg p-6 border border-gray-200 dark:border-gray-700">
      ...
      <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
        <div className="flex items-center text-sm text-gray-600 dark:text-gray-400">
          ...
          <span>{highRiskPhases} phases need attention</span>
        </div>
      </div>
    </div>
  );
